...
/*
 * TaskDAO supports create, findById, findAll (with user/category names),
 * update, delete, and a helper to list tasks by user.
 */
public class TaskDAO {
    ...
    public Task findById(int id) throws Exception {
        ...
        try (Connection c = DBUtil.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return new Task(rs.getInt("task_id"), rs.getInt("user_id"),
    ...
    public List<Task> findAll() throws Exception {
        ...
        try (Connection c = DBUtil.getConnection();
            ...
            while (rs.next()) {
                list.add(new Task(rs.getInt("task_id"), rs.getInt("user_id"), rs.getInt("category_id"),
    ...
    public boolean update(Task t) throws Exception {
    ...
    public List<String> findAllWithNames() throws Exception {
        // returns simple formatted strings with user/category names for listing UI
        ...
        try (Connection c = DBUtil.getConnection();
            ...
            while (rs.next()) {
                ...
                out.add(String.format("%d) [%s] (%s) - %s%s", id, u, cat, desc, done ? " [DONE]" : ""));
            }
        }
        return out;
    }
